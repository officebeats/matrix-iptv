...
use crate::handlers::input::is_m3u_url;
...
pub fn render_login(f: &mut Frame, app: &App, area: Rect) {
    ...
    // Detect whether the current URL is an M3U playlist to visually de-emphasize credentials
    let url_value = app.input_url.value();
    let is_m3u = is_m3u_url(url_value)
        || app
            .editing_account_index
            .and_then(|i| app.config.accounts.get(i))
            .map(|a| a.account_type.is_m3u())
            .unwrap_or(false);
    ...
    // Show [M3U] badge in the URL label when an M3U URL is detected
    let url_label = if is_m3u {
        "server url  [M3U - no credentials needed]"
    } else {
        "server url"
    };
    f.render_widget(
    ...
    // For M3U playlists, dim the username/password fields to indicate they are not used
    if is_m3u {
        f.render_widget(
            render_input_dimmed("username  (not used for M3U)", app.input_username.value()),
            chunks[current_chunk],
        );
        current_chunk += 1;
        f.render_widget(
            render_input_dimmed("password  (not used for M3U)", ""),
            chunks[current_chunk],
        );
    } else {
        f.render_widget(
...
/// Renders a form input field that is visually dimmed/disabled (e.g., credentials for M3U).
fn render_input_dimmed<'a>(label: &'a str, _value: &'a str) -> Paragraph<'a> {
    let label_style = Style::default().fg(TEXT_DIM);
    let lines = vec![
        Line::from(Span::styled(format!("  {}", label), label_style)),
        Line::from(Span::styled("  n/a", label_style)),
    ];
    Paragraph::new(lines)
}
    ...
    match app.settings_state {
        ...
        SettingsState::ManageAccounts => {
            ...
            let accounts: Vec<ListItem> = app
                ...
                .iter()
                .map(|acc| {
                    let type_badge = if acc.account_type.is_m3u() {
                        " [M3U]"
                    } else {
                        ""
                    };

...
fn render_sports_categories_pane(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let (start, end) = visible_window(
        app.sports_category_list_state.selected().unwrap_or(0),
        app.sports_categories.len(),
        visible_height,
    );
...
fn render_sports_matches_pane(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let inner_area = crate::ui::common::render_matrix_box_active(
        f,
        area,
        &title_padded,
        border_color,

use crate::app::App;
use crate::ui::colors::{HIGHLIGHT_BG, MATRIX_GREEN, TEXT_DIM, TEXT_PRIMARY, TEXT_SECONDARY};
use ratatui::{
...
pub fn render_home(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let logo_lines: Vec<Line> = vec![
        ...
        Line::from(vec![Span::styled(
            "  Terminal streaming client",
            Style::default().fg(TEXT_DIM),
        )]),
    ];

    f.render_widget(
        Paragraph::new(logo_lines).bg(crate::ui::colors::MODERN_BG),
        main_layout[0],
    );
    ...
    let accounts: Vec<ListItem> = app
        ...
        .map(|acc| {
            ...
            // Show account type badge for non-Xtream accounts
            if acc.account_type.is_m3u() {
    ...
    let playlist_title = Paragraph::new(Line::from(vec![Span::styled(
        "  playlists",
        Style::default()
            .fg(MATRIX_GREEN)
            .add_modifier(Modifier::BOLD),
    ...
    if app.config.accounts.is_empty() {
        guides_text.extend(vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    "  Understanding the IPTV Protocol",
                    Style::default().fg(TEXT_PRIMARY),
                ),
            ]),
            Line::from(""),

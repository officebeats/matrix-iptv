    ...
    match client.get_live_categories().await {
        Ok(categories) => {
            ...
            let target_cat = categories
                .iter()
                .find(|c| c.category_name.contains("NBA") || c.category_name.contains("Sports"))
                .or_else(|| {
                    categories.iter().find(|c| {
                        c.category_name.contains("USA")
                            || c.category_name.contains(" Entertainment")
                    })
                })
                        ...
                        if let Some(s) = stream {
                            ...
                            for ext in exts {
                                ...
                                match player.play(&test_url, engine, use_default, smooth).await {
                                    Ok(_) => {
                                        ...
                                        for i in 1..=6 {
                                            ...
                                            if !player.is_running() {
                                                if let Some(log_err) =
                                                    player.get_last_error_from_log()
                                                {
                                                    println!(
                                                        "    ✗ Player exited. Log error: {}",
                                                        log_err
                                                    );
                                                } else {
                                                    println!(
                                                        "    ✗ Player exited (no error in log)"
                                                    );
                                                }
                                                break;
                                            }
                                    ...
                                    Err(e) => {
                                        ...
                                        let diagnosis =
                                            player.diagnose_playback_failure(&e.to_string());
                            ...
                            if !success {
                                ...
                                if let Some(vlc_path) = matrix_iptv_lib::setup::get_vlc_path() {
                                    ...
                                    let result =
                                        std::process::Command::new(&vlc_path).arg(&vlc_url).spawn();
